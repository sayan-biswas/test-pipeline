...
// clusterReconcilerReconcilerGenerator produces a reconciler struct for the given type.
type clusterReconcilerReconcilerGenerator struct {
...
var _ generator.Generator = (*clusterReconcilerReconcilerGenerator)(nil)

func (g *clusterReconcilerReconcilerGenerator) Filter(c *generator.Context, t *types.Type) bool {
	// Only process the type for this generator.
	return t == g.typeToGenerate
}

func (g *clusterReconcilerReconcilerGenerator) Namers(c *generator.Context) namer.NameSystems {
	return namer.NameSystems{
		"raw": namer.NewRawNamer(g.outputPackage, g.imports),
	}
}

func (g *clusterReconcilerReconcilerGenerator) Imports(c *generator.Context) (imports []string) {
	imports = append(imports, g.imports.ImportLines()...)
	return
}

func (g *clusterReconcilerReconcilerGenerator) GenerateType(c *generator.Context, t *types.Type, w io.Writer) error {
	...
	m := map[string]interface{}{
		...
		"logicalClusterName": c.Universe.Type(types.Name{
			Package: "github.com/kcp-dev/logicalcluster/v2",
			Name:    "Name",
		}),
		...
		// Deps
		"clientsetInterface": c.Universe.Type(types.Name{Name: "ClusterInterface", Package: g.clientsetPkg}),
	...
	sw.Do(clusterReconcilerInterfaceFactory, m)
	sw.Do(clusterReconcilerNewReconciler, m)
	sw.Do(clusterReconcilerImplFactory, m)
	if len(g.reconcilerClasses) > 1 {
		sw.Do(clusterReconcilerLookupClass, m)
	}
	if g.hasStatus {
		sw.Do(clusterReconcilerStatusFactory, m)
	}
	sw.Do(clusterReconcilerFinalizerFactory, m)

	return sw.Error()
}

var clusterReconcilerLookupClass = `
...
var clusterReconcilerInterfaceFactory = `
...
var clusterReconcilerNewReconciler = `
...
var clusterReconcilerImplFactory = `
...
func (r *reconcilerImpl) Reconcile(ctx {{.contextContext|raw}}, key string) error {
	...
	{{if .nonNamespaced}}
	getter := r.Lister.Cluster(s.cluster)
	{{else}}
	getter := r.Lister.Cluster(s.cluster).{{.type|apiGroup}}(s.namespace)
	{{end}}
...
var clusterReconcilerStatusFactory = `
func (r *reconcilerImpl) updateStatus(ctx {{.contextContext|raw}}, cluster {{.logicalClusterName|raw}}, existing *{{.type|raw}}, desired *{{.type|raw}}) error {
	existing = existing.DeepCopy()
	return {{.reconcilerRetryUpdateConflicts|raw}}(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {
			{{if .nonNamespaced}}
			getter := r.Client.Cluster(cluster).{{.group}}{{.version}}().{{.type|apiGroup}}()
			{{else}}
			getter := r.Client.Cluster(cluster).{{.group}}{{.version}}().{{.type|apiGroup}}(desired.Namespace)
			{{end}}
		...
		{{if .nonNamespaced}}
		updater := r.Client.Cluster(cluster).{{.group}}{{.version}}().{{.type|apiGroup}}()
		{{else}}
		updater := r.Client.Cluster(cluster).{{.group}}{{.version}}().{{.type|apiGroup}}(existing.Namespace)
		{{end}}
...
var clusterReconcilerFinalizerFactory = `
// updateFinalizersFiltered will update the Finalizers of the resource.
// TODO: this method could be generic and sync all finalizers. For now it only
// updates defaultFinalizerName or its override.
func (r *reconcilerImpl) updateFinalizersFiltered(ctx {{.contextContext|raw}}, cluster {{.logicalClusterName|raw}}, resource *{{.type|raw}}, desiredFinalizers {{.setsString|raw}}) (*{{.type|raw}}, error) {
	// Don't modify the informers copy.
	...
	{{if .nonNamespaced}}
	patcher := r.Client.Cluster(cluster).{{.group}}{{.version}}().{{.type|apiGroup}}()
	{{else}}
	patcher := r.Client.Cluster(cluster).{{.group}}{{.version}}().{{.type|apiGroup}}(resource.Namespace)
	{{end}}
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx {{.contextContext|raw}}, cluster {{.logicalClusterName|raw}}, resource *{{.type|raw}}) (*{{.type|raw}}, error) {
	...
	return r.updateFinalizersFiltered(ctx, cluster, resource, finalizers)
}

func (r *reconcilerImpl) clearFinalizer(ctx {{.contextContext|raw}}, cluster {{.logicalClusterName|raw}}, resource *{{.type|raw}}, reconcileEvent {{.reconcilerEvent|raw}}) (*{{.type|raw}}, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
