...
import (
	...
	pipelinev1alpha1 "github.com/tektoncd/pipeline/pkg/client/cluster/listers/tekton/v1alpha1"
	zap "go.uber.org/zap"
...
type reconcilerImpl struct {
	...
	Lister pipelinev1alpha1.RunClusterLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.ClusterInterface, lister pipelinev1alpha1.RunClusterLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.Cluster(s.cluster).Runs(s.namespace)
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, cluster logicalcluster.Name, existing *v1alpha1.Run, desired *v1alpha1.Run) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.Cluster(cluster).TektonV1alpha1().Runs(desired.Namespace)
		...
		updater := r.Client.Cluster(cluster).TektonV1alpha1().Runs(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, cluster logicalcluster.Name, resource *v1alpha1.Run) (*v1alpha1.Run, error) {

	getter := r.Lister.Cluster(cluster).Runs(resource.Namespace)
	...
	patcher := r.Client.Cluster(cluster).TektonV1alpha1().Runs(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, cluster logicalcluster.Name,resource *v1alpha1.Run) (*v1alpha1.Run, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, cluster logicalcluster.Name, resource *v1alpha1.Run, reconcileEvent reconciler.Event) (*v1alpha1.Run, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
