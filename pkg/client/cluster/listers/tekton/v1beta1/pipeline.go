...
type PipelineClusterLister interface {
	List(selector labels.Selector) (ret []*tektonv1beta1.Pipeline, err error)
Cluster(cluster logicalcluster.Name)tektonv1beta1listers.PipelineLister
}
...
func (s *pipelineClusterLister) List(selector labels.Selector) (ret []*tektonv1beta1.Pipeline, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*tektonv1beta1.Pipeline))
	})
	return ret, err
}

// Cluster scopes the lister to one workspace, allowing users to list and get Pipelines.
func (s *pipelineClusterLister) Cluster(cluster logicalcluster.Name)tektonv1beta1listers.PipelineLister {
return &pipelineLister{indexer: s.indexer, cluster: cluster}
}

// pipelineLister implements the tektonv1beta1listers.PipelineLister interface.
...
func (s *pipelineLister) List(selector labels.Selector) (ret []*tektonv1beta1.Pipeline, err error) {
	...
	for i := range list {
		obj := list[i].(*tektonv1beta1.Pipeline)
		if selectAll {
			ret = append(ret, obj)
		} else {
...
func (s *pipelineLister) Pipelines(namespace string) tektonv1beta1listers.PipelineNamespaceLister {
return &pipelineNamespaceLister{indexer: s.indexer, cluster: s.cluster, namespace: namespace}
}

// pipelineNamespaceLister implements the tektonv1beta1listers.PipelineNamespaceLister interface.
...
func (s *pipelineNamespaceLister) List(selector labels.Selector) (ret []*tektonv1beta1.Pipeline, err error) {
	selectAll := selector == nil || selector.Empty()
...
func (s *pipelineNamespaceLister) Get(name string) (*tektonv1beta1.Pipeline, error) {
	...
	if !exists {
		return nil, errors.NewNotFound(tektonv1beta1.Resource("Pipeline"), name)
	}
	return obj.(*tektonv1beta1.Pipeline), nil
}
